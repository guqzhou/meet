<template>
  <div id="signOut">
    ...
    <footer>
      <van-form>
        <ul>
          ...
          <li>
            <van-field label-align="right" readonly v-model="form.time2" label="签离时间:" />
          </li>
        </ul>
        <div class="btn">
    ...
    <!--    <div style="margin:22px;width: 20px;height: 20px;"></div>-->
  ...
  methods: {
    ...
    submit() {
      ...
      const params = {
        ...
        leaveTime: this.form.time2
      };
      this.$axios.post("meet/meetSign/signOut", params).then(res => {
        console.log(res);
        if (res.data.code === 0) {
    ...
    validate() {
      ...
      if (this.form.phone) {
        this.$axios.post("meet/meetSign/getList", params).then(res => {
          const { data } = res.data;
          this.form.time1 = data.signinTime;
          if (data.sealFixedOrgId === "-1") {
            this.choicePlace = data.unitName;
          } else {
            this.choicePlace = data.orgName;
          }
          this.partmentId = data.sealFixedOrgId;
        });
      }
    }
  }
